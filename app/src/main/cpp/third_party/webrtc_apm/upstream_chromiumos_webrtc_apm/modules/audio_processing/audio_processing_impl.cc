...
bool AudioProcessingImpl::CreateAndAttachAecDump(absl::string_view file_name,
                                                 ...
                                                 TaskQueueBase*
                                                     worker_queue) {
  (void)file_name;
  (void)max_log_size_bytes;
  (void)worker_queue;
  // AEC dump is disabled in the Android standalone minimal APM build.
  return false;
}
...
// Android standalone minimal APM link stubs.
namespace webrtc {

void WarnThatTheCurrentThreadIsProbablyDeadlocked() {
  // No-op for Android standalone WebRTC APM build.
}
...
InternalAPMConfig& InternalAPMConfig::operator=(const InternalAPMConfig& other) {
  (void)other;
  return *this;
}
